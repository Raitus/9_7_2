...
int main() {
  ...
  if (firstNumber[0] - '0' >= 0 && firstNumber[0] - '0' <= 9 && firstNumber[1] - '0' >= 0 && firstNumber[1] - '0' <= 9
	  && firstNumber[2] - '0' >= 0 && firstNumber[2] - '0' <= 9 && firstNumber[3] - '0' >= 0
	  && firstNumber[3] - '0' <= 9) {
	int cow = 0, bull = 0;
	...
	std::cout << "Bulls: " << bull << ", cows: " << cow << ".\n"<<firstNumber.length()<<std::endl<<secondNumber.length();
  } else {
